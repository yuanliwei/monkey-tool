async function start() {
    ...
    let tree = null
    while (i--) {
        await monkey.sleep(`800`)
        tree = await monkey.getVisibleTree()
        await monkey.clickText(tree, '退出登录')
        await monkey.sleep(`500`)
        await monkey.clickRect([563,1236,863,1350])
        await monkey.sleep(`300`)
        await monkey.clickRect([111,862,879,1018])//password
        await monkey.sleep(`300`)
        await monkey.run(`type 123456`)
        await monkey.sleep(`300`)
        tree = await monkey.getVisibleTree()
        await monkey.clickRect([111,1144,969,1288])//login
        await monkey.sleep(`1000`)
        tree = await monkey.getVisibleTree()
        await monkey.clickRect([765,2263,855,2302])
        await monkey.sleep(`300`)
        tree = await monkey.getVisibleTree()
        await monkey.clickText(tree, '设置')
        await monkey.sleep(`1000`)
